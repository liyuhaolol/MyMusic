package spa.lyh.cn.lib_audio.mediaplayer.core;
...
public class AudioPlayer
  ...
  private void init() {
    ...
    mWifiLock = ((WifiManager) AudioHelper.getContext()
        ...
        .getSystemService(Context.WIFI_SERVICE)).createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, TAG);
    // 初始化音频焦点管理器
  ...
  public void load(AudioBean audioBean) {
    try {
      mMediaPlayer.reset();//清空上一次的数据
      mMediaPlayer.setDataSource(audioBean.mUrl);//设置数据
      mMediaPlayer.prepareAsync();//准备异步播放
      //发送加载音频事件，UI类型处理事件
